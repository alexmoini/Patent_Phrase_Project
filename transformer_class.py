...
class SimilarityModel(torch.nn.Module):
    ...
    def forward(self, anchor_input_ids, anchor_attention_mask, target_input_ids, target_attention_mask):
        ...
        print(type(encoded_anchor))
        print(type(anchor_attention_mask))

        # mean pooling
        anchor_pooled_output = self.mean_pooling(encoded_anchor.last_hidden_state, anchor_attention_mask)
        target_pooled_output = self.mean_pooling(encoded_target.last_hidden_state, target_attention_mask)
        # normalize
